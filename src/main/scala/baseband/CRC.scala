...
class CRC extends Module {
  val io = IO(new Bundle {
    ...
    /*
    TODO: Both ready and valid of result were always set to true.B, so it was refactored to a non-decoupled interface.
          Correctness verification is needed.
    */
  ...
  val lfsr = RegInit(0.U(24.W)) // x^24 + x^10 + x^9 + x^6 + x^4 + x^3 + x^1 + 1
