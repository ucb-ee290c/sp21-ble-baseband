...
class GFSKRXTest extends AnyFlatSpec with ChiselScalatestTester {
  ...
  def RFtoIF(in: Seq[Double]): Seq[(Double, Double)] = {
    ...
    val rf = {t:Double => math.cos(2 * math.Pi * (F_RF + gausFirData((t / 1e-7).floor.toInt) * 0.25 * MHz) * t + math.Pi / 4)}
